...
// ═══════════════ Slide 01 — Cover ═══════════════
{
  ...
  s.addText("{{발송일자}} · 담당 파트너 Demian (임정훈)", {
...
// ═══════════════ Slide 03 — 고객사 진단 ═══════════════
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
  addTitle(s, 3, "{{고객사명}} 상황 진단", "사전 인터뷰 기반 — 우리가 이해한 현재");
...
// ═══════════════ Slide 07 — 5패키지 매트릭스 ═══════════════
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
  addTitle(s, 7, "5패키지 매트릭스", "{{고객사명}} 맞춤 — {{추천패키지}}");
  ...
  s.addText("{{고객사명}} 추천: {{추천패키지}}", { x: 1.0, y: 5.4, w: 11.4, h: 0.5, fontSize: 18, bold: true, color: NAVY, fontFace: FONT_H, margin: 0 });
  s.addText("선정 근거: {{선정근거}}   ·   포함 Phase: {{포함Phase}}   ·   투입: {{투입MD}} MD", {
    x: 1.0, y: 5.95, w: 11.4, h: 0.4, fontSize: 13, color: NAVY_DARK, fontFace: FONT_B, margin: 0
  });
  s.addText("산출물: {{산출물목록}}", { x: 1.0, y: 6.4, w: 11.4, h: 0.4, fontSize: 13, color: GRAY, fontFace: FONT_B, italic: true, margin: 0 });
...
// ═══════════════ Slide 12 — 보안 & 거버넌스 ═══════════════
{
  ...
  s.addText([
    ...
    { text: "방법론·프레임워크 = FLOW~ 귀속  ·  고객사 데이터·결과물 = {{고객사명}} 귀속", options: { color: NAVY_DARK, fontSize: 12 } }
...
// ═══════════════ Slide 14 — 투자 & Next Step ═══════════════
{
  ...
  s.addText("{{고객사명}}의 AX, 끝까지 함께 가겠습니다.", { x: 1.5, y: 0.68, w: 11.5, h: 0.35, fontSize: 13, fontFace: FONT_B, color: ICE, margin: 0 });
  ...
  s.addText("{{견적}}", { x: 0.9, y: 5.15, w: 5.6, h: 1.0, fontSize: 36, bold: true, color: NAVY, fontFace: FONT_H, margin: 0 });
  ...
  const steps = [
    ...
    { n: "3", t: "SOW 확정 + Kickoff", d: "{{기간}} 전환 여정 시작" }
  ];
  steps.forEach((st, i) => {
    const y = 2.3 + i * 1.25;
  ...
  s.addText([
    ...
    { text: "{{이메일}} · {{연락처}}", options: { color: ICE, fontSize: 11 } }
...
pres.writeFile({ fileName: "FLOW_AX_Master_Proposal.pptx" })
